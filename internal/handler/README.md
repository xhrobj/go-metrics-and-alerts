...
Пакет содержит HTTP-обработчики Сервера. Handler преобразует HTTP-запросы в вызовы `service`, формирует ответы и отправляет события аудита после успешного обновления метрик.
...
Модель JSON описана в [`internal/model`](../model/README.md).
...
Middleware доверенной подсети подключено только к маршрутам записи:
