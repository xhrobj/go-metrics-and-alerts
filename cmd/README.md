...
Для локального запуска `Makefile` при наличии автоматически читает файл `.env`. Создайте его из примера:

```bash
cp .env.example .env
```

После этого значения в `.env` можно изменить под локальное окружение. Сам файл `.env` игнорируется Git и не должен попадать в репозиторий. Агент и Сервер не читают env-файл напрямую: `Makefile` передает значения существующим флагам и переменным окружения.

Все команды продолжают работать и без `.env`, используя значения по умолчанию из `Makefile`. Разово переопределить параметр можно в команде:

```bash
make run-server TRUSTED_SUBNET=10.0.0.0/8
```

При необходимости можно подключить другой env-файл:
