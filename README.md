...
Это основной учебный проект курса [«Продвинутый Go-разработчик»](https://practicum.yandex.ru/go-advanced/) от Яндекс Практикума. В этом README сохранены исходные формулировки заданий и история 28 инкрементов, выполненных в течение восьми спринтов: [с 1-го по 4-й] и [с 6-го по 9-й].

Курс также включает два итоговых проекта: [(^.^)~ Gophermart](https://github.com/xhrobj/gophermart) в спринте 5 и [(^-^)/ GophKeeper](https://github.com/xhrobj/gopherkeeper) в спринте 10.

Актуальная информация о текущей реализации, конфигурации и способах запуска проекта **«Сервис сбора метрик и алертинга»** находится в [описании приложений и способов их запуска](cmd/README.md).

Ниже сохранена хронология развития проекта по спринтам и инкрементам.

> Разделы по инкрементам отражают состояние проекта на момент выполнения соответствующего задания. Указанные в них пути, команды и названия пакетов могут отличаться от текущей структуры.
...
- #48. Добавить ретраи 429/5xx HTTP-ошибок Агента

  при ревью, попросили добавить ошибки 429 и 5xx в логику ретраев (см. Спринт 3, Инкремент 13, Задача #37)

**Архитектурные заметки**

Сбор runtime- и системных метрик выполняется независимо и обновляет общее хранилище. Каждые `REPORT_INTERVAL` Агент формирует полный пакет метрик и помещает его в очередь отправки, если после предыдущего отчёта был выполнен хотя бы один опрос.

Пакеты обрабатываются пулом воркеров, число которых ограничено значением `RATE_LIMIT`.

Так как запросы могут завершаться не в том порядке, в котором были поставлены в очередь, более старый пакет теоретически может перезаписать на Сервере более новые значения gauge-метрик. При стандартных интервалах это маловероятно, но риск возрастает при сетевых задержках и заполнении очереди.
...
Итоговый проект: [**(^.^)~ Gophermart**](https://github.com/xhrobj/gophermart).
...
Основным сценарием для оптимизации выбран пакетный обработчик `/updates`.
...
```text
http://localhost:8080/github.com/xhrobj/go-metrics-and-alerts/internal/server/transport/http/router#pkg-examples
```
...
В `pkg/pool` добавлен типизированный пул объектов с методом `Reset()`. Для пакета добавлены GoDoc-описание и исполняемый пример использования.
...
- #79. Добавить проверку заголовка X-Real-IP на стороне Сервера

**Результаты**

При запуске Сервера с доверенной подсетью `192.168.1.0/24` запросы от Агента успешно проходят проверку и обрабатываются со статусом `200 OK`.

При запуске Сервера с подсетью, в которую IP-адрес Агента не входит:

```bash
make run-server TRUSTED_SUBNET=10.0.0.0/8
```

Сервер отклоняет запросы Агента со статусом `403 Forbidden`, что видно в логах:

```json
{"level":"info","ts":1781613203.237355,"caller":"middleware/logging.go:68","msg":"http request completed","uri":"/updates","method":"POST","duration":0.000179125,"status":403,"size":0}
```
...
3. Передача IP-адреса. Для проверки подсети на сервере агенту необходимо передавать свой IP-адрес. В gRPC для этого используются метаданные. Агент должен добавлять свой IP-адрес в метаданные запроса, например, с ключом x-real-ip.
4. Проверка подсети. На сервере должен быть реализован UnaryInterceptor, который проверяет, принадлежит ли IP-адрес агента доверенной подсети (trusted_subnet). Если проверка не пройдена, сервер должен возвращать ошибку с кодом codes.PermissionDenied.
5. Конфигурация. Все новые параметры (например, адрес gRPC-сервера) должны настраиваться через переменные окружения и флаги командной строки, аналогично существующим параметрам.

**Задачи**

- #80. Выделить сервисный слой в Агенте
- #81. Отделить Аудит от HTTP-транспорта на Сервере
- #82. Добавить protobuf-контракт и генерацию gRPC-кода
- #83. Добавить gRPC-транспортный слой на Сервере
- #84. Интегрировать gRPC-транспорт в конфигурацию и жизненный цикл Сервера
- #85. Реализовать и интегрировать gRPC-транспорт в Агенте

---

### Итоги спринта 9

<a href="docs/images/sprint9-evo.png">
  <img src="docs/images/previews/sprint9-evo.png" alt="Спринт 9 - эволюция сервиса">
</a>
...
Итоговый проект: [**(^-^)/ GophKeeper**](https://github.com/xhrobj/gopherkeeper).

---

### Итоги спринтов 6 - 10

<a href="docs/images/sprint6-10-path.png">
  <img src="docs/images/previews/sprint6-10-path.png" alt="Спринты 6-10 - карта инкрементов">
</a>
