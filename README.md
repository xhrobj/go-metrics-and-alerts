...
- отправить несторого согласованный снепшот

В задании просят реализовать отправку через worker pool, получается:

- каждые `REPORT_INTERVAL` report() формирует (но только если счётчик pollSinceReport не 0) полный batch и кладет его в очередь
...
Не очень понятный момент:

- ???: какая задача (или обе?) инкрементирует pollSinceReport?..

... в общем, пока так - сейчас report() завязан только на runtime-сборщик:

- отправка происходит только когда был хотя бы один runtime-poll
- системные метрики просто "едут вместе" в общем snapshot.
